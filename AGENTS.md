...
`docs/ARCHITECTURE.md` and explicitly approved project decisions override generic architecture guidance in third-party skills. The approved production blueprint preserves `GolfBall`, `LevelBuilder`, `LevelValidator`, authored-hole fallback, flat 2D physics, and signal boundaries while evolving the project incrementally. QFramework, CQRS, ECS, Controller/System/Model/Utility, Command/Query, event-bus, global-state rewrite, and similar patterns require explicit project adoption; their presence in the third-party `$godot` skill is not approval to introduce them.
...
- Prefer named, centralized tuning values and established data boundaries. Use `@export` only when editor tuning is useful.
...
- Never automatically close an issue. Meaningful work is complete only when the repository lifecycle grants explicit closing authority; automated verification is not authority for subjective quality or completion.
- When issue closure is explicitly authorized, use `chainlink issue close --no-changelog` unless the repository deliberately adopts a changelog workflow. Chainlink-generated `CHANGELOG.md` is not repository documentation.
